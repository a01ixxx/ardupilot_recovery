...
Vector3f _last_gyro_filtered[INS_MAX_INSTANCES];
...
{    
    ...
    if (_imu._new_gyro_data[instance]) {
        _last_gyro_filtered[instance] = _imu._gyro_filtered[instance];
...
void AP_InertialSensor_Backend::update_gyro_with_stale_data(uint8_t instance)
{    
    WITH_SEMAPHORE(_sem);
    ...
    if (_imu._new_gyro_data[instance]) {
        _publish_gyro(instance, _last_gyro_filtered[instance]);
        _imu._new_gyro_data[instance] = false;
    }
...
void AP_InertialSensor_Backend::update_gyro_with_noise(uint8_t instance)
{    
    WITH_SEMAPHORE(_sem);
    ...
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dis(1.0, 1.2);
    // Generate a random number in the range [1.0, 1.2]
    float random1 = dis(gen);
    float random2 = dis(gen);
    float random3 = dis(gen);

    // Vector3f factor(random1, random2, random3);

    if (_imu._new_gyro_data[instance]) {
        _publish_gyro(instance, 
                      Vector3f(random1 * _imu._gyro_filtered[instance].x,
                               random2 * _imu._gyro_filtered[instance].y,
                               random3 * _imu._gyro_filtered[instance].z));
        _imu._new_gyro_data[instance] = false;
    }
...
Vector3f last_accel_filtered[INS_MAX_INSTANCES];
...
{    
    ...
    if (_imu._new_accel_data[instance]) {
        last_accel_filtered[instance] = _imu._accel_filtered[instance];
...
/*
  common accel update function for all backends
 */
void AP_InertialSensor_Backend::update_accel_with_stale_data(uint8_t instance)
{    
    WITH_SEMAPHORE(_sem);
    ...
    // printf("staled data x: %f, y: %f, z:%f\n", last_accel_filtered[instance].x, last_accel_filtered[instance].y, last_accel_filtered[instance].z);

    if (_imu._new_accel_data[instance]) {
        _publish_accel(instance, last_accel_filtered[instance]);
        _imu._new_accel_data[instance] = false;
    }
...
/*
  common accel update function for all backends
 */
void AP_InertialSensor_Backend::update_accel_with_noise(uint8_t instance)
{    
    WITH_SEMAPHORE(_sem);
    ...
    if (_imu._new_accel_data[instance]) {
        _publish_accel(instance,
                       Vector3f(random1 * _imu._accel_filtered[instance].x,
                                random2 * _imu._accel_filtered[instance].y,
                                random3 * _imu._accel_filtered[instance].z));
        _imu._new_accel_data[instance] = false;
    }
