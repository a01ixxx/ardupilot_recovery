...
ByteBuffer _last_readbuffer{16384};
...
int16_t UARTDriver::read(void)
{
    ...
    uint8_t c;
    // // _last_readbuffer = _readbuffer;
    // memcpy(_last_readbuffer.buf, _readbuffer.buf, sizeof(_readbuffer.size));
    // _last_readbuffer.size = _readbuffer.size;
    // _last_readbuffer.head = _readbuffer.head.load();
    // _last_readbuffer.tail = _readbuffer.tail.load();

    _readbuffer.read(&c, 1);
    return c;
}

int16_t UARTDriver::read_stale(void)
{
    if (available() <= 0) {
        return -1;
    }
    uint8_t c;
    _last_readbuffer.read(&c, 1);
    return c;
}
